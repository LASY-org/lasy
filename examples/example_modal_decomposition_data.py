...
intensityData = np.roll(np.roll(intensityData,-int(xCOM-cols/2),axis=1),-int(yCOM-rows/2),axis=0)


pixel_calib = 0.186e-6
lo = (-intensityData.shape[0]/2*pixel_calib, -intensityData.shape[0]/2*pixel_calib)
hi = (intensityData.shape[0]/2*pixel_calib, intensityData.shape[0]/2*pixel_calib)
...
profile = CombinedLongitudinalTransverseProfile( wavelength, pol, energy,
                                                 longProfile, transProf)


# Calculate the decomposition into hermite-gauss modes
n_x_max = 20
n_y_max = 20
modeCoeffs, wasit = hermite_gauss_decomposition(transProf,n_x_max=n_x_max,n_y_max=n_y_max,N_pts=500)
...
for i, mode_key in enumerate(list(modeCoeffs)):
    ...
    if i == 0:
        reconstructedProfile = modeCoeffs[mode_key] * \
            CombinedLongitudinalTransverseProfile( wavelength, pol,
                                                  energy, longProfile, tmpTranProf )
    else:
        reconstructedProfile += modeCoeffs[mode_key] * \
            CombinedLongitudinalTransverseProfile( wavelength, pol, energy,
                                                  longProfile, tmpTranProf)
...
fig.suptitle('Hermite-Gauss Reconstruction using n_x_max = %i, n_y_max = %i' %(n_x_max,n_y_max))
plt.show()