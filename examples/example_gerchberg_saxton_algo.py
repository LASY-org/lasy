...
phaseBackward, phaseForward,amp_error = gerchberg_saxton_algo(
    laserBackward,laserForward, 2*propDist,condition='amplitude_error', 
    max_itterations=50,amplitude_error=1e-6,debug=True)
...
im0 = ax[0,0].imshow(np.abs(laser.field.field[:,:,tIndx])**2,extent=extent,cmap='PuRd')
addColorbar(im0,ax[0,0],'Intensity (norm.)')
ax[0,0].set_title('Inten. z = 0.0 mm')
ax[0,0].set_xlabel('x ($\mu m$)');ax[0,0].set_ylabel('y ($\mu m$)')


im1 = ax[0,1].imshow(np.angle(laser.field.field[:,:,tIndx])*phaseMask,extent=extent,cmap='coolwarm')
addColorbar(im1,ax[0,1],'Phase (rad.)')
ax[0,1].set_title('Phase z = 0.0 mm')
ax[0,1].set_xlabel('x ($\mu m$)');ax[0,1].set_ylabel('y ($\mu m$)')
...
im2 = ax[1,0].imshow(np.abs(np.abs(laser.field.field[:,:,tIndx])**2-np.abs(laser_calc.field.field[:,:,tIndx])**2),
               extent=extent,cmap='PuRd')
ax[1,0].set_title('Inten. Res. z = 0.0 mm')
ax[1,0].set_xlabel('x ($\mu m$)');ax[1,0].set_ylabel('y ($\mu m$)')
addColorbar(im2,ax[1,0],'Intensity (norm.)')

phaseResidual = np.angle(laser_calc.field.field[:,:,tIndx])-np.angle(laser.field.field[:,:,tIndx])
phaseResidual -= phaseResidual[int(npoints[1]/2),int(npoints[0]/2)]
maxPhaseRes = np.max(np.abs(phaseResidual)*phaseMask)
im3 = ax[1,1].imshow(phaseResidual*phaseMask,extent=extent,cmap='coolwarm',vmin=-maxPhaseRes,vmax=maxPhaseRes)
ax[1,1].set_title('Phase Res. z = 0.0 mm')
ax[1,1].set_xlabel('x ($\mu m$)');ax[1,1].set_ylabel('y ($\mu m$)')
addColorbar(im3,ax[1,1],'Phase (rad.)')


im4 = ax[0,2].imshow(np.abs(laserBackward.field.field[:,:,tIndx])**2,extent=extent,cmap='PuRd')
addColorbar(im4,ax[0,2],'Intensity (norm.)')
ax[0,2].set_title('Inten. z = %.1f mm' %(-propDist*1e3))
ax[0,2].set_xlabel('x ($\mu m$)');ax[0,2].set_ylabel('y ($\mu m$)')

im5 = ax[0,3].imshow(np.angle(laserBackward.field.field[:,:,tIndx]),extent=extent,cmap='coolwarm')
addColorbar(im5,ax[0,3],'Phase (rad.)')
ax[0,3].set_title('Phase z = %.1f mm' %(-propDist*1e3))
ax[0,3].set_xlabel('x ($\mu m$)');ax[0,3].set_ylabel('y ($\mu m$)')

phaseResidual = np.angle(laserBackward.field.field[:,:,tIndx])-phaseBackward[:,:,tIndx]
phaseResidual -= phaseResidual[int(npoints[1]/2),int(npoints[0]/2)]
maxPhaseRes = np.max(np.abs(phaseResidual)*phaseMask)
...
ax[1,2].set_xlabel('x ($\mu m$)');ax[1,2].set_ylabel('y ($\mu m$)')
im8 = ax[1,3].imshow(np.angle(laserForward.field.field[:,:,tIndx]),extent=extent,cmap='coolwarm')
addColorbar(im8,ax[1,3],'Phase (rad.)')
ax[1,3].set_title('Phase z = %.1f mm' %(propDist*1e3))
ax[1,3].set_xlabel('x ($\mu m$)');ax[1,3].set_ylabel('y ($\mu m$)')

phaseResidual = np.angle(laserForward.field.field[:,:,tIndx])-phaseForward[:,:,tIndx]
phaseResidual -= phaseResidual[int(npoints[1]/2),int(npoints[0]/2)]
maxPhaseRes = np.max(np.abs(phaseResidual)*phaseMask)
