...
class LongitudinalProfileFromData(LongitudinalProfile):
    ...
    def __init__(self, data, lo, hi):
        ...
        if data['datatype'] == 'spectral':
            ...
            dt = data['dt']
            cwl = np.sum(spectral_intensity*wavelength
                        )/np.sum(spectral_intensity)
            cfreq = c/cwl
            ...
            freq = np.linspace(cfreq-sample_freq/2,cfreq+sample_freq/2
                                , N)
            # interpolate the spectrum onto this new array
            freq_intensity = np.interp(freq,c/wavelength[::-1],
                                        spectral_intensity[::-1],
                                        left=0, right=0)
            freq_phase = np.interp(freq,c/wavelength[::-1],
                                        spectral_phase[::-1],
                                        left=0, right=0)
            ...
            t_amplitude = np.fft.fftshift(np.fft.ifft(
                np.fft.ifftshift(freq_amplitude*
                                np.exp(-1j*freq_phase))))/dt
