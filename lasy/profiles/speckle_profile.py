...
class SpeckleProfile(Profile):
    ...
    * Induced spatial incoherence (ISI), ``'GP ISI'``:
        a smoothing technique with temporal stochastic variation in the beamlet complex amplitudes; that is, :math:`\phi_{{\rm CPP},ml}=0`, :math:`\psi_{{\rm SSD},ml}(t)=0`, and the :math:`A_{ml}` are complex amplitudes sampled from a Gaussian stochastic process to simulate the random phase differences and amplitudes the beamlets pick up passing through the ISI echelons
    ...
    def beamlets_complex_amplitude(
        ...
        if temporal_smoothing_type.upper() == "FM SSD":
            phase_t = self.ssd_phase_modulation_amplitude[0] * np.sin(
                self.ssd_x_y_dephasing[0] * self.ssd_phase_modulation_frequency[0]
                * (
