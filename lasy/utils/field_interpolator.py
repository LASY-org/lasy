...
def interpolate_complex_field_XY(spectral_field, X, Y, OM, X_new, Y_new):
    ...
    for k in range(Nom):
        ...
        if not (np.allclose(X[:, :, k], np.meshgrid(xk, yk, indexing='ij')[0]) and
                np.allclose(Y[:, :, k], np.meshgrid(xk, yk, indexing='ij')[1])):
            raise ValueError(f"X and Y slice {k} are not regular grids.")

        # Create fast interpolators
        interp = RegularGridInterpolator((xk, yk), spectral_field[:, :, k], method='nearest', bounds_error=False, fill_value=0)

        # Interpolate on target grid
        target_points = np.stack([X_new[:, :, k].ravel(), Y_new[:, :, k].ravel()], axis=-1)
