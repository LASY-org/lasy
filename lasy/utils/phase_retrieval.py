...
class GerchbergSaxton:
    ...
    def initialise_spotsizes(self, spotsizes=None):
        ...
        if spotsizes is None:
            w0x, w0y = estimate_best_HG_waist(
                self.x,
                self.y,
                np.sum(self.amps[self.z0idx], axis=-1),
                self.lasers[self.z0idx].profile.lambda0,
    ...
    def initialise_modes(self):
        ...
        if self.initial_phase is None:
            ...
            z_R = (
                np.pi
                * np.mean([self.spotsizes[0], self.spotsizes[1]]) ** 2
                / self.lasers[self.z0idx].profile.lambda0
            )
            self.phase = (
                np.pi
        ...
        self.lasers[self.z0idx].grid.set_temporal_field(
            self.amps[self.z0idx] * np.exp(1j * self.phase)
        )

        # Find estimate of the decomposition of the initial electric field
        self.modes = hermite_gauss_decomposition(
            self.lasers[self.z0idx],
            self.spotsizes[0],
            self.spotsizes[1],
            self.m_max,
            self.n_max,
            z_foc=self.z[self.z0idx],
        )
                ...
                if calcModeKey[0] < 3 and calcModeKey[1] < 3:
                    print(
                        ...
                        % (
                            calcModeKey[0],
                            calcModeKey[1],
                            np.real(self.modes[calcModeKey]),
                            np.imag(self.modes[calcModeKey]),
                        )
                    )
    ...
    def retrieve_phase(self):
        ...
        for i in range(self.max_iter):
            if self.verbose:
                print("GSA Iteration: %i" % i)
            for k in idx:
                if self.verbose:
                    print("    Image: %i of %i" % (k + 1, len(self.z)))
                ...
                t1 = time.time()
                if self.verbose:
                    print("        Reconstruction: %.3f s" % (t1 - t0))

                # Step 3
                phase = np.angle(self.lasers[k].grid.get_temporal_field())

                # Step 4
                laser_new = self.amps[k] * np.exp(
                    1j * phase
                )  # Enforce amplitude condition

                # Step 5
                delta = (
                    self.amps[k] - np.abs(self.lasers[k].grid.get_temporal_field())
                ) / np.max(self.amps[k])
                ...
                new_modes = hermite_gauss_decomposition(
                    self.lasers[k],
                    self.spotsizes[0],
                    self.spotsizes[1],
                    self.m_max,
                ...
                t1 = time.time()
                if self.verbose:
                    print("        Decomposition:  %.3f s" % (t1 - t0))

                for key in new_modes:
                    new_modes[key] *= np.sqrt(
                        mode_power
                        / sum(abs(value) ** 2 for value in new_modes.values())
                    )
                    self.modes[key] = (self.modes[key] + new_modes[key]) / 2.0

                for key in self.modes:
                    self.modes[key] *= np.sqrt(
                        mode_power
                        / sum(abs(value) ** 2 for value in self.modes.values())
                    )

                # calculate the fluence error
                chi2[i] = (
                    np.sqrt(np.sum((np.abs(laser_new) - self.amps[k]) ** 2))
                    / np.sum(self.amps[k])
                    / len(self.z)
                )

            if i > 0:
                chi2Grad[i] = np.abs(chi2[i] - chi2[i - 1]) / (
                    chi2[i - 1] + 1e-8
                )  # Add a small amount to prevent inf
                if self.verbose:
