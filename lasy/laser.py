...
from lasy.utils.field import Field
...
class Laser:
    ...
    def propagate(self, distance, nr_boundary=None, backend="NP"):
        ...
        if nr_boundary is not None:
            ...
            if self.dim == "rt":
                self.field.array[:, -nr_boundary:, :] *= absorb_layer_shape[
                    None, :, None
                ]
            else:
                self.field.array[-nr_boundary:, :, :] *= absorb_layer_shape[
                ...
                self.field.array[:nr_boundary, :, :] *= absorb_layer_shape[::-1][
                    :, None, None
                ]
                self.field.array[:, -nr_boundary:, :] *= absorb_layer_shape[
                    None, :, None
        ...
        field_fft = np.fft.ifft(self.field.array, axis=time_axis_indx, norm="backward")

        # Create the frequency axis
        dt = self.field.dx[time_axis_indx]
        ...
        if self.dim == "rt":
            ...
            if not hasattr(self, "prop"):
                spatial_axes = (self.field.axes[0],)
                self.prop = []
        ...
        # Translate the box
        self.field.lo[time_axis_indx] += translate_time
        self.field.hi[time_axis_indx] += translate_time
        self.field.axes[time_axis_indx] += translate_time
    ...
    def export_to_z(self, z_axis=None, z0=0.0, t0=0.0, backend="NP"):
        ...
        t_axis = self.field.axes[time_axis_indx]
        if z_axis is None:
            z_axis = t_axis * c
        ...
        if self.dim == "rt":
            ...
            field_z = np.zeros(
                (self.field.array.shape[0], self.field.array.shape[1], z_axis.size),
                dtype=self.field.array.dtype,
            )
            ...
            # Convert the spectral image to the spatial field representation
            FieldAxprp.import_field(np.transpose(self.field.array).copy())
    ...
    def import_from_z(self, field_z, z_axis, z0=0.0, t0=0.0, backend="NP"):
        ...
        """
        z_axis_indx = -1
        t_axis = self.field.axes[z_axis_indx]
        ...
        if self.dim == "rt":
            ...
            for i_m in range(self.box.azimuthal_modes.size):
                ...
                self.field.array[i_m] *= np.exp(1j * (z0 / c + t_axis) * omega0)
        else:
            # Construct the propagator
            Nx, Ny, Nt = self.field.array.shape
            Lx = self.box.hi[0] - self.box.lo[0]
