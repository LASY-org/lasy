...
def Gaussian(x, y, t):
    ...
    z = z_foc - vf * (
        t - t_peak
    )  # make the focal plane time dependent for flying focus
    ...
    envelope = (
        ...
        * np.exp(
            -np.power(((t - t_peak) ** 2) / tau**2, n_order / 2)
        )  # pulse spread term
        * np.exp(1.0j * (cep_phase + omega0 * t_peak))  # phase factor
        * z_r
        * 1.0j
        / q  # normalization, the 1.0j accounts for a phase built into lasy
    )
...
def HermiteGaussian(x, y, t):
    ...
    hermite_norm_x = np.sqrt(
        np.sqrt(2 / np.pi) / (2 ** hg_modes[0] * factorial(hg_modes[0]) * waist_x)
    )  # coef from x
    hermite_norm_y = np.sqrt(
        np.sqrt(2 / np.pi) / (2 ** hg_modes[1] * factorial(hg_modes[1]) * waist_y)
    )  # coef from y
...
def LaguerreGaussian(x, y, t):
    ...
    phi = np.arctan2(y, x)
    q = z + 1.0j * z_r  # complex factor
    gouy = (2 * lg_modes[0] + np.abs(lg_modes[1]) + 1) * np.arctan2(
        z, z_r
    )  # gouy phase
    ...
    laguerre_norm = (
        np.sqrt(
            2
            * factorial(lg_modes[0])
            / (np.pi * factorial(lg_modes[0] + np.abs(lg_modes[1])))
        )
        / waist
    )  # should normalize
...
def HermiteGaussian2D(x, t):
    ...
    hermite_norm_x = np.sqrt(
        np.sqrt(2 / np.pi) / (2**hg_modes * factorial(hg_modes) * waist_x)
    )  # coef from x
...
laser_profile_ff_gauss = ParaxialFlyingFocusGaussianProfile(
    3, w_0, wavelength, pol, energy, tau, t_peak, vf, cep_phase, z_foc, n_order
)
...
error = 100 * (
    rel_error_real + rel_error_imag
)  # error in percent which sums the real and imaginary errors
...
E_analytical_gauss = E_analytical_gauss / np.max(
    np.abs(E_analytical_gauss)
)  # normed analytical solution for Gauss
...
E_analytical_ff_gauss_2d = E_analytical_ff_gauss_2d / np.max(
    np.abs(E_analytical_ff_gauss_2d)
)

# initialize 2d Gauss FF implementation
laser_profile_ff_gauss_2d = ParaxialFlyingFocusGaussianProfile(
    2, w_0, wavelength, pol, energy, tau, t_peak, vf, cep_phase, z_foc, n_order
)
...
# error calculations
rel_error_real = np.max(
    np.abs(np.real(E_analytical_ff_gauss_2d) - np.real(E_lasy_ff_gauss_2d))
)
rel_error_imag = np.max(
    np.abs(np.imag(E_analytical_ff_gauss_2d) - np.imag(E_lasy_ff_gauss_2d))
)
error = 100 * (
    rel_error_real + rel_error_imag
)  # error in percent which sums the real and imaginary errors
...
E_analytical_hg = E_analytical_hg / np.max(
    np.abs(E_analytical_hg)
)  # normed analytical solution for HG
...
# error calculations
rel_error_real = np.max(
    np.abs(np.real(E_analytical_ff_hg_2d) - np.real(E_lasy_ff_hg_2d))
)
rel_error_imag = np.max(
    np.abs(np.imag(E_analytical_ff_hg_2d) - np.imag(E_lasy_ff_hg_2d))
)
error = 100 * (
    rel_error_real + rel_error_imag
)  # error in percent which sums the real and imaginary errors
...
E_analytical_lg = E_analytical_lg / np.max(
    np.abs(E_analytical_lg)
)  # normed analytical solution for LG
