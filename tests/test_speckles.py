...
def test_intensity_distribution():
    ...
    e_i = np.imag(F)
    er_ei = e_r * e_i
    assert np.max(abs(e_r.mean(axis=(0,1)) / e_r.std(axis=(0,1)))) < 8.e-2
    assert np.max(abs(e_i.mean(axis=(0,1)) / e_i.std(axis=(0,1)))) < 8.e-2
    assert np.max(abs(er_ei.mean(axis=(0,1)) / er_ei.std(axis=(0,1)))) < 8.e-2
...
def test_spatial_correlation():
    ...
    x_list = np.linspace(-n_beamlets[0] / 2 + 0.5, n_beamlets[0] / 2 - 0.5,num_points[0],endpoint=False)
    y_list = np.linspace(-n_beamlets[1] / 2 + 0.5, n_beamlets[1] / 2 - 0.5,num_points[1],endpoint=False)
...
def test_FM_SSD_periodicity():
    ...
    phase_mod_freq = [relative_laser_bandwidth * sf * 0.5 / pma for sf, pma in zip(ssd_frac, phase_mod_amp)]
    t_max = 1./phase_mod_freq[0] / nu_laser
