...
def check_axicon(laser):
    ...
    new_laser = Laser(laser.dim, laser.grid.lo, laser.grid.hi, laser.grid.npoints, gaussian_profile)
...
def check_bessel_profile(laser, z):
    ...
    expected_profile = 4 * power * k * np.sin(gamma)/w0 * np.exp(-2 * z**2/z_max**2) * z/z_max * special.j0(kr)**2
