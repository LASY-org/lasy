...
def test_nonlinear_step():
    ...
    for z in z_pos:
        ...
        bandwidth = get_bandwidth(grid=laser.grid, dim=laser.dim, method='on-axis') * 2 # times 2 to convert half-width to full width
        bandwidths_propagated.append(bandwidth)

    # calculate peak intensity and max phase shift for analytical broadening calculation
    peak_intensity = np.max(0.5 * c * epsilon_0 * abs(laser.grid.get_temporal_field()) ** 2)
    ...
    bandwidths_analytical = initial_bandwidth * np.sqrt(1+4/(3*3**0.5)*(phase_shift)**2)
