...
def test_longitudinal_profiles():
    ...
    t_flat = 12e-15
    t_down = 8e-15
    profile_flat_top_linear = FlatTopLongitudinalProfile(
        wavelength, t_start, t_rise, t_flat, t_down, cep_phase, "linear"
    )
    profile_flat_top_cos2 = FlatTopLongitudinalProfile(
        wavelength, t_start, t_rise, t_flat, t_down, cep_phase, "cos2"
    )

    tft = np.linspace(
        t_start - 2.0e-15, (t_start + t_rise + t_flat + t_down) + 2.0e-15, npoints
    )
    ...
    std_flat_top_linear = np.sqrt(
        np.average((ttft - t_fc) ** 2, weights=np.abs(field_flat_top_linear))
    )
    std_flat_top_cos2 = np.sqrt(
        np.average((tft - t_fc) ** 2, weights=np.abs(field_flat_top_cos2))
    )
    ...
    assert (
        np.abs(std_flat_top_linear - std_flat_top_linear_th) / std_flat_top_linear_th
        < 0.01
    )
    assert (
        np.abs(std_flat_top_cos2 - std_flat_top_cos2_th) / std_flat_top_cos2_th < 0.01
    )
